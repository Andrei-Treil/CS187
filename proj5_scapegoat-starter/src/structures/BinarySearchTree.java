...
public class BinarySearchTree<T extends Comparable<T>> implements BSTInterface<T> {
  ...
  private T getHelper(T t, BSTNode<T> node) {
		...
		else if (result < 0) return getHelper(t, node.getLeft());
  ...
  private T getMinHelper(BSTNode<T> node){
    if (node.getLeft() == null) return node.getData();
		else return getMinHelper(node.getLeft());
  }
  ...
  public Iterator<T> preorderIterator() {
    ...
    Queue<T> queue = new LinkedList<T>();
		preorderTraverser(queue, root);
		return queue.iterator();
  }

  private void preorderTraverser(Queue<T> queue, BSTNode<T> node){
    if(node != null){
      queue.add(node.getData());
      preorderTraverser(queue, node.getLeft());
      preorderTraverser(queue, node.getRight());
    }
  }
  ...
  public Iterator<T> postorderIterator() {
    ...
    Queue<T> queue = new LinkedList<T>();
		postorderTraverser(queue, root);
		return queue.iterator();
	}
	
	private void postorderTraverser(Queue<T> queue, BSTNode<T> node) {
		if (node != null) {
			postorderTraverser(queue, node.getLeft());
			postorderTraverser(queue, node.getRight());
			queue.add(node.getData());
		}
	}
  ...
  public void balance() {
	...
	private BSTNode<T> arrToBST(T[] arr, int lower, int upper) {
		if (lower > upper) return null;
		int mid = (lower + upper)/2;
		BSTNode<T> newNode = new BSTNode<T>(arr[mid], arrToBST(arr, lower, mid - 1), arrToBST(arr, mid + 1, upper));
		return newNode;
	}
